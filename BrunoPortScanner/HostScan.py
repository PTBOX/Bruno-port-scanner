...
q = Queue()
reader = csv.DictReader(open('services.csv'))
services = {}
for row in reader:
    services[f"{row['Port Number']}({row['Transport Protocol']})"] = row['Service Name']
...
def threader():
    while True:
        port_data = q.get()
        tcp_scanner(port_data)
        q.task_done()


def tcp_scanner(address_port_tuple):
    address_to_scan, port, socket_timeout, max_retries, results, debug = address_port_tuple
    sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    sock.settimeout(socket_timeout)
    for i in range(max_retries):
        try:
            result = sock.connect_ex((address_to_scan, port))
            if result == 0:
                with print_lock:
                    try:
                        service = services[f"{port}(tcp)"]
                        if debug:
            ...
            print(e)
            print(f"Connection to {address_to_scan} failed")
            return
...
def main(address_to_scan, ports_to_scan, num_of_threads, consider_alive, max_retries, debug, socket_timeout):
    open_ports = {}
    latency = ping(address_to_scan) if not consider_alive else True
    open_ports[address_to_scan] = []
    if latency:
        address_to_scan = socket.gethostbyname(address_to_scan)
        start_time_address = time.time()
        if debug:
            print(f"scanning: {address_to_scan} using timeout: {socket_timeout}")
        ...
        for port in ports_to_scan:
            q.put((address_to_scan, port, socket_timeout, max_retries, open_ports[address_to_scan], debug))
        q.join()
        if debug:
            print(f"Scanned {address_to_scan} in {(time.time() - start_time_address)} Seconds")
            print(f"------------------------{address_to_scan}------------------------")
    else:
        if debug:
            print(f"skipped {address_to_scan} probably dead use -a to consider it alive")
    print(json.dumps(open_ports))


def get_ports(top_ports):
    if top_ports:
        return [1, 3, 4, 6, 7, 9, 13, 17, 19, 20, 21, 22, 23, 24, 25, 26, 30, 32, 33, 37, 42, 43, 49, 53, 70, 79, 80,
                ...
                10082, 10180, 10215, 10243, 10566, 10616, 10617, 10621, 10626, 10628, 10629, 10778, 11110, 11111, 11967,
                12000, 12174, 12265, 12345, 13456, 13722, 13782, 13783, 14000, 14238, 14441, 14442, 15000, 15002, 15003,
                15004, 15660, 15742, 16000, 16001, 16012, 16016, 16018, 16080, 16113, 16992, 16993, 17877, 17988, 18040,
                18101, 18988, 19101, 19283, 19315, 19350, 19780, 19801, 19842, 20000, 20005, 20031, 20221, 20222, 20828,
                21571, 22939, 23502, 24444, 24800, 25734, 25735, 26214, 27000, 27352, 27353, 27355, 27356, 27715, 28201,
                30000, 30718, 30951, 31038, 31337, 32768, 32769, 32770, 32771, 32772, 32773, 32774, 32775, 32776, 32777,
                32778, 32779, 32780, 32781, 32782, 32783, 32784, 32785, 33354, 33899, 34571, 34572, 34573, 35500, 38292,
                40193, 40911, 41511, 42510, 44176, 44442, 44443, 44501, 45100, 48080, 49152, 49153, 49154, 49155, 49156,
                49157, 49158, 49159, 49160, 49161, 49163, 49165, 49167, 49175, 49176, 49400, 49999, 50000, 50001, 50002,
...
if __name__ == '__main__':
    ...
    parser.add_argument('-a', '--alive', help='Consider hosts as alive, default is false', action='store_true',
    ...
    address = args['target']
    num_of_threads = args['rate']
    consider_alive = args['alive']
    max_retries = args['max_retries']
    top_ports = args['top_ports']
    debug = args['debug']
    ports = get_ports(top_ports)
    socket_timeout = args['socket_timeout']
    main(address, ports, num_of_threads, consider_alive, max_retries, debug, socket_timeout)
    if debug:
        print(f"Scan of {address} done in: {(time.time() - start_time)} Seconds")
